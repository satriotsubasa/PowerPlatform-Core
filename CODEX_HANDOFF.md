...
- v1.3.0 also fixes headless plug-in **assembly** registration. The classic database-assembly path does NOT auto-create `plugintype` records (the Plug-in Registration Tool creates them via reflection), so `register-assembly` now creates them **explicitly** from the step type names via `EnsureAssemblyPluginTypes` (it was polling 60s in `WaitForAssemblyPluginTypes` for an auto-discovery that never happens, then dead-ending; re-runs hit "assembly already exists"). New `reuseExistingAssembly` spec flag / `--reuse-existing` reconciles an assembly left by a partial run (idempotent types + steps; skips existing steps by plugintype+message+stage) without re-uploading content. The package path is unchanged (Dataverse creates types server-side) and is now positioned as the more reliable default. `plugins-server-extensions` skill + reference updated; `PluginTypeRegistration.BuildPluginTypeEntity` unit-tested. No version change - this lands inside the still-untagged 1.3.0. (Shipped as v1.3.1: the manifests were bumped to force plugin hosts to re-cache, since they cache by version.)
- v1.4.0 (branch `feature/platform-administration`): new **12th skill** `platform-administration` for Power Platform tenant/environment admin (the admin-center tasks), via `pac admin` + the Power Platform API (.NET `Microsoft.PowerPlatform.Management` / Python `powerplatform-management` SDKs). Different auth tier (Power Platform Administrator, NOT the maker/dev Dataverse auth) and a stricter **admin preflight** than the data preflight. Phased: **Phase 1 (this release) is read-only** - `admin_inspect.py` (`--mode environments|tenant-settings|dlp-policies|groups`) via `pac admin`, JSON-enveloped, with an admin-auth hint on failure. Phase 2 = safe writes (create env, tenant settings, managed environments) behind the admin preflight; Phase 3 = high-risk (env delete/reset/restore, DLP create/edit/delete). Cross-platform: use `pac admin` + the API/SDKs, NOT the Windows-only `*.Administration.PowerShell` module. **Power BI editing was evaluated and explicitly declined** for this plugin - separate domain (PBIP/TMDL/Fabric APIs, paid-capacity-gated, no `pac`/Dataverse reuse). The general Web API executor is deferred to a planned v1.4.0 — the plugin still makes NO direct Web API calls (SDK `ServiceClient` + `pac` CLI only).

- July 2026: full competitive analysis vs Microsoft's **Dataverse MCP server** (5 research passes over official docs, release plans, community ground truth, and a neutral repo audit). Verdict: complements, not competitors - the MCP owns conversational data access + central governance; this plugin owns repo-first delivery (flows/plug-ins/PCF/ALM/safety). Public summary: `docs/comparison-dataverse-mcp.md`; full internal analysis: `marketing/dataverse-mcp-full-analysis.md` (gitignored, local). Outcome: `docs/roadmap.md` defines **v1.5.0 "Trust & Interop"** - Phase A service-principal auth, Phase B enforced preflight tokens (top-5 mutating helpers), Phase C Dataverse-MCP routing rules, Phase D positioning/bootstrap/dry-run. Admin skill Phase 2/3 stays gated on maintainer go-ahead; Power BI remains explicitly out of scope.
