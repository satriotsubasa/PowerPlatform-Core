#!/usr/bin/env python3
"""Create a Dataverse lookup relationship with the shared SDK metadata helper."""
...
from powerplatform_common import apply_selected_solution_to_spec, read_json_argument, resolve_live_connection, run_dataverse_tool


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create a Dataverse lookup relationship through the shared SDK metadata helper.",
    )
    parser.add_argument("--spec", required=True, help="JSON object or path to a JSON file describing the lookup.")
    ...
    parser.add_argument("--auth-dialog", action="store_true", help="Launch the reusable auth dialog before creating the lookup.")
