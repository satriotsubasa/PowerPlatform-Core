#!/usr/bin/env python3
"""Create or update a Dataverse web resource from a local file."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create or update a Dataverse web resource from a local file through the shared SDK helper.",
    )
    parser.add_argument("--spec", required=True, help="JSON object or path to a JSON file describing the web resource sync.")
    parser.add_argument("--repo-root", default=".", help="Repository root used to resolve relative file paths inside the spec.")
    ...
    parser.add_argument("--auth-dialog", action="store_true", help="Launch the reusable auth dialog before syncing the web resource.")
    ...
    if "filePath" in spec and isinstance(spec["filePath"], str):
        candidate = Path(spec["filePath"])
        if not candidate.is_absolute():
            spec["filePath"] = str((repo / candidate).resolve())
