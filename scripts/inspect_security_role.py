#!/usr/bin/env python3
"""Inspect or list Dataverse security roles through the shared SDK helper."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Inspect or list Dataverse security roles through the shared SDK helper.",
    )
    ...
    parser.add_argument("--auth-dialog", action="store_true", help="Launch the reusable auth dialog before inspecting security roles.")
    ...
    if mode not in {"inspect", "list"}:
        print("ERROR: inspect_security_role only supports mode 'inspect' or 'list'.", file=sys.stderr)
        return 2
