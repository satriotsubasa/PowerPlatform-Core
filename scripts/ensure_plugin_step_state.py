#!/usr/bin/env python3
"""Reconcile Dataverse plug-in step enablement for an existing assembly or package."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Reconcile Dataverse plug-in step enablement for an existing assembly or package.",
    )
    parser.add_argument("--spec", required=True, help="JSON object or path to a JSON file describing the desired step states.")
    ...
    parser.add_argument("--auth-dialog", action="store_true", help="Launch the reusable auth dialog before reconciling plug-in step states.")
    ...
    final_spec = apply_selected_solution_to_spec(spec, connection)

    with tempfile.NamedTemporaryFile("w", delete=False, suffix=".json", encoding="utf-8") as handle:
        handle.write(json.dumps(final_spec, indent=2))
