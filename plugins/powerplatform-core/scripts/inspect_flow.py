#!/usr/bin/env python3
"""Inspect or list solution-aware cloud flows through the shared Dataverse SDK helper."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Inspect or list solution-aware cloud flows through the shared Dataverse SDK helper.",
    )
    parser.add_argument("--spec", required=True, help="JSON object or path to a JSON file describing the inspect or list request.")
    ...
    parser.add_argument("--auth-dialog", action="store_true", help="Launch the reusable auth dialog before inspecting the flow.")
    ...
    if mode not in {"inspect", "list"}:
        print("ERROR: inspect_flow only supports mode 'inspect' or 'list'.", file=sys.stderr)
        return 2
    ...
    try:
        ...
        if mode == "list":
            if spec.get("solutionUniqueName"):
                command.extend(["--solution-unique-name", str(spec["solutionUniqueName"])])
            if spec.get("includeClientData"):
                command.append("--include-client-data")
        else:
