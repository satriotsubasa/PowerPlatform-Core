#!/usr/bin/env python3
"""Inspect or read Dataverse environment variable values through the shared SDK helper."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Inspect or read Dataverse environment variable definitions and current values.",
    )
    parser.add_argument("--spec", required=True, help="JSON object or path to a JSON file describing the inspect request.")
    ...
    parser.add_argument("--auth-dialog", action="store_true", help="Launch the reusable auth dialog before inspecting the environment variable.")
    ...
    mode = str(spec.get("mode") or "inspect").strip().lower()
    if mode not in {"inspect", "get-value"}:
        print("ERROR: inspect_environment_variable only supports mode 'inspect' or 'get-value'.", file=sys.stderr)
        return 2
