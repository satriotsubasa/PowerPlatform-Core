#!/usr/bin/env python3
"""Create a Dataverse environment variable DEFINITION through the shared SDK helper.

Complements set_environment_variable_value.py (which only sets values on existing
definitions). The spec is a JSON object with schemaName (publisher-prefixed),
displayName, type (string|number|boolean|json|datasource|secret), and optional
description, defaultValue, value (an initial current value), solutionUniqueName, and
allowPrefixMismatch.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create a Dataverse environment variable definition through the shared SDK helper.",
    )
    parser.add_argument("--spec", required=True, help="JSON object or path to a JSON file describing the environment variable definition.")
    parser.add_argument("--repo-root", default=".", help="Repository root used as the working directory for the shared SDK helper.")
    ...
    parser.add_argument("--auth-dialog", action="store_true", help="Launch the reusable auth dialog before creating the definition.")
